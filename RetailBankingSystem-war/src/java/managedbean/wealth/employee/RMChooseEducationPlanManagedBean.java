 ...
 * @author Jingyuan
 */
@Named(value = "rMChooseEducationPlanManagedBean")
@RequestScoped
public class RMChooseEducationPlanManagedBean {

    /**
     * Creates a new instance of RMChooseEducationPlanManagedBean
     */
    @EJB
    ...
    public RMChooseEducationPlanManagedBean() {
    }
    ...
    public void chooseWealthPlan(ActionEvent event) {
        ...
        if (contributeDuration < 5) {
            message = new FacesMessage(FacesMessage.SEVERITY_ERROR, "Contribute duration for retirement plan must be over 5 years", null);
            context.addMessage(null, message);
        } else if (worthYear < contributeDuration) {
            message = new FacesMessage(FacesMessage.SEVERITY_ERROR, "Investment period cannot be shorter than contribute duration", null);
            context.addMessage(null, message);
        } else {
            createFinancialGoalSessionBeanLocal.createPortfolio(id, startingBalance, monthlyContribution, contributeDuration, worthYear, amountExpected, "education plan",rate);
